 ...
 *
 * <p>Production wires a real {@code HttpBreetServiceImpl} that uses the
 * Breet API key. The {@code @Profile("!prod")} annotation keeps this
 * stub out of prod by default.</p>
 */
@Slf4j
@Service
@Profile("!prod")
public class BreetServiceStubImpl implements BreetService {
    ...
    public void handleWebhook(BreetWebhookPayload payload) {
        // For the stub, record the payload as a settled deposit so the
        // mobile poll picks it up. Real impl would: verify address ownership,
        // persist the deposit, credit the wallet, log the audit row.
